...
async function fixUnavailableVideos() {
  console.log('🔧 Fixing unavailable YouTube videos...');
  ...
  for (const video of videos) {
    ...
    if (ytMatch) {
      ...
      if (!isAvailable) {
        // Get a reliable video ID (cycle through the list)
        const validId = RELIABLE_VIDEO_IDS[fixedCount % RELIABLE_VIDEO_IDS.length];
        ...
        if (fixedCount % 50 === 0) {
          console.log(`\n📊 Fixed ${fixedCount} videos so far...\n`);
        }
      }
    }
  }

  console.log(`\n🎉 Fixed ${fixedCount} unavailable video URLs`);
  
  await prisma.$disconnect();
}

fixUnavailableVideos();
