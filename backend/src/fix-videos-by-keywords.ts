...
// Verified embeddable videos organized by topic keywords
const VIDEOS_BY_KEYWORD = {
...
function findVideosByKeywords(title: string): string[] {
  ...
  for (const [keyword, videos] of Object.entries(VIDEOS_BY_KEYWORD)) {
  ...
  return VIDEOS_BY_KEYWORD['default'];
}

async function fixVideosByKeywords() {
  console.log('🔧 Fixing videos with keyword-matched content...');
  ...
  try {
    ...
    console.log(`\n🎉 Updated ${updatedCount} video URLs with keyword-matched content!`);

  } catch (error) {
    console.error('❌ Error fixing videos by keywords:', error);
  } finally {
