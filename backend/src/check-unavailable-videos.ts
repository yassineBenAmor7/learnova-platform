...
// List of reliable YouTube video IDs that are known to work
...
async function checkUnavailableVideos() {
  console.log('🔍 Checking for unavailable YouTube videos...');
  ...
  const unavailableVideos: Array<{ id: number; title: string; url: string; sessionId: number; videoId: string }> = [];
  let checkedCount = 0;
  ...
  for (const video of videos) {
    ...
    if (ytMatch) {
      ...
      if (checkedCount % 50 === 0) {
        console.log(`Checked ${checkedCount}/${videos.length} videos...`);
      }
    }
  }

  console.log(`\n📊 Total videos checked: ${checkedCount}`);
  console.log(`❌ Unavailable videos: ${unavailableVideos.length}`);
  
  if (unavailableVideos.length > 0) {
    console.log('\n📋 Unavailable video list:');
    ...
    console.log('\n💡 To fix these videos, run: npx ts-node src/fix-unavailable-videos.ts');
  } else {
